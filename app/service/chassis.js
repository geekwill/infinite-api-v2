...
class ChassisService extends Service {
  ...
  async search(params) {
    ...
    const sqlArray = [];
    sqlArray.push('SELECT DISTINCT ch.key, ch.images, ch.name, ch.brand, ch.batch, ch.model, ch.company_name as companyName');
    sqlArray.push('FROM (SELECT * FROM chassis WHERE 1 = 1');
    // 基础参数条件
    ...
    sqlArray.push(`ORDER BY batch DESC LIMIT ${(params.page - 1) * 10}, 10) as ch`);
    // 链接表
